...
if (!auth_can_manage_announcement(auth_user(), $row)) {
  http_response_code(403);
  $err_title = 'Accès refusé';
  $err_text = 'Tu ne peux modifier que tes propres annonces (ou tout en tant qu’admin).';
  $err_style = 'warning';
  $err_show_home = true;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($action === 'delete') {
    ...
    db()->prepare('DELETE FROM announcements WHERE id = :id')->execute(['id' => (int)$id]);
    auth_log($uid, 'delete_announcement', 'announcement', (int)$id, null);
    header('Location: index.php?route=announcements');
    exit;
  }
  ...
  if ($title === '' || $content === '') {
    ...
    if (!$up['ok']) {
      ...
      db()->prepare(
        'UPDATE announcements SET title=:t, content=:c, image_path=:i, category_slug=:cat WHERE id=:id'
      )->execute([
        't' => $title,
        'c' => $content,
        'i' => $newPath,
      ...
      auth_log($uid, 'update_announcement', 'announcement', (int)$id, null);
      $success = 'Annonce enregistrée.';
      $row = getAnnouncementRowById((int)$id) ?? $row;
      $savedPath = (string)($row['image_path'] ?? '');
    }
  }
}
...
<section class="my-6 max-w-2xl mx-auto">
  ...
  <p class="text-sm text-base-content/60 mb-4">Nouvelle pièce jointe : JPG, PNG ou PDF (max 5 Mo), glisser-déposer ou clic sur la zone.</p>
  ...
  <form method="post" enctype="multipart/form-data" class="card bg-base-200/50 border border-base-content/10 p-5 gap-3 flex flex-col">
    ...
    <select class="select select-bordered" name="category_slug" required>
      <?php foreach (ANNOUNCEMENT_CATEGORIES as $slug => $label): ?>
        <option value="<?= h($slug) ?>" <?= (string)$row['category_slug'] === $slug ? 'selected' : '' ?>><?= h($label) ?></option>
      <?php endforeach; ?>
    </select>
    <?php if ($savedPath !== ''): ?>
    ...
    <?php endif; ?>
    <input type="file" id="ann-file-edit" name="attachment" class="hidden" accept=".jpg,.jpeg,.png,.pdf,image/jpeg,image/png,application/pdf">
    <div id="ann-dropzone-edit" class="rounded-xl border-2 border-dashed border-base-content/25 bg-base-100/50 px-4 py-8 text-center text-sm text-base-content/65 cursor-pointer hover:border-primary/40 transition-colors">
    ...
    <div class="flex flex-wrap gap-2">
      ...
      <a class="btn btn-ghost border border-base-content/15" href="index.php?route=announcement_detail&id=<?= (int)$id ?>">Voir l’annonce</a>
    </div>
  </form>
  <form method="post" class="mt-6" onsubmit="return confirm('Supprimer définitivement cette annonce ?');">
    <input type="hidden" name="action" value="delete">
    <button class="btn btn-outline border-error/40 text-error" type="submit">Supprimer l’annonce</button>
  </form>
</section>
<script>
(function () {
  var z = document.getElementById('ann-dropzone-edit');
  var inp = document.getElementById('ann-file-edit');
  if (!z || !inp) return;
